...
-   High-Level Profiling (Extensions).
-   Tabular Quality Control (CSV Check).
-   Image Quality Control (Metadata and Fixity).
-   HDF5 (Hierarchies).
-   NetCDF (Arrays).
-   PDF (Security and Text).
-   SQLite (Schemas).
:::
:::::
...
Our curators are not preservationists. We organize, describe, and clean files, and we encourage preservation-friendly formats, but we do not perform additional long-term preservation activities. Digital Preservation services works on these elements.
:::
...
- **Format Diversity:** Hundreds of unique extensions, including `csv`, `pdf`, `jpg`, and `xlsx`.
:::
...
<div style="font-size: 0.65em; margin-top: 5px; color: #555; font-style: italic;">FRDR interactive curation pipeline. Working with the researcher is an essential element of this project</div>
:::
:::::
...
4. **Varying Documentation:** Missing READMEs, orphaned variables, and obscure custom scripts.
   
5. **Reproducibility and Security:**Checking code reproducibility, scanning for hardcoded paths, and identifying potential Personally Identifying Information (PII) (emails/keys) at scale.
...
-   **Problems Solved:**
    ...
    -   *Triage:* Estimating volume and complexity.
:::
:::
-   **Key Tools Used:**
...
::: {.column style="width: 50%; font-size: 0.65em;"}
::: {.r-fit-text}
-   **Why:** Tabular files often suffer from parsing errors, missing data, and Personally Identifying Information (PII) leaks.
-   **Obtained:**
...
-   **Problems Solved:**
    ...
    -   *Format Obsolescence:* Flagging proprietary raw formats for conversion to preservation-friendly TIFFs.
:::
:::
-   **Key Tools Used:**
...
## Implementation in HPC clusters

This toolbox provides R-scripts that can be executed in HPC cluters like [Fir](https://docs.alliancecan.ca/wiki/Fir) from the Digital Research Alliance of Canada.

An example can be found [here](https://github.com/Alliance-RDM-GDR/CUR_Res_CurationTools/tree/main/workshop/HPC)
...
- [Explora portal](https://explora.alliancecan.ca/)
:::
::::
...
![](images/qr-code.png){fig-alt="QR code linking to the published workshop presentation" width="220px"}
