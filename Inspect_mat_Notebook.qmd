...
MAT-files are the proprietary binary data container format used by MATLAB. They are widely used in engineering but pose a significant challenge for long-term preservation.
...
-   **Format Obsolescence (v5 vs v7.3):** The version column identifies the file architecture. v7.3 are essentially HDF5 files, but still require specialized libraries (rhdf5 in R, h5py in Python) to read. Consider creating a codebook that maps the HDF5 paths (e.g., /group1/dataset_A) to their scientific meaning.

-   **Workspace Dumps:** Files with high object counts (\>50) and generic names (var1, temp, data_copy) are likely RAM demanding. Consider requesting a data dictionary (if not already present in the dataset), or a cleaned version containing only the final variables.

-   **Complex Structures (H5T_COMPOUND):** In v7.3 files, H5T_COMPOUND indicates complex nested tables (structs). These are difficult to convert to CSV. If you are converting files, consider converting these specific tables to JSON or separate CSVs to preserve the hierarchy.
