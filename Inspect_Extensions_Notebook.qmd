---
title: "File extensions and format inspector"
...
This notebook performs the initial "Triage" of a data package by inventorying file extensions. In digital preservation, it is crucial to understand that a **File Extension** (e.g., `.csv`, `.pdf`) is merely a textual label and does not guarantee the file's internal format.

A robust preservation workflow distinguishes between:

1.  **Extension-based Identification:** Fast but unreliable. A user can rename `virus.exe` to `photo.jpg`. Curators must remain aware that extensions can be misleading.

2.  **Signature-based Identification:** Verifying the "Magic Number" (hexadecimal signature) embedded in the file header. For example, all Java class files begin with `CA FE BA BE`.

**Curation Objectives:**

1.  **Content Profiling:** Establish the composition of the dataset (e.g., 80% Tabular Data, 20% Proprietary Code).
2.  **Junk Detection:** Identify system artifacts (`.DS_Store`, `Thumbs.db`) that clutter the archive.
3.  **Risk Flagging:** Detect executables (`.exe`, `.bat`) or zero-byte files that indicate transfer errors.
