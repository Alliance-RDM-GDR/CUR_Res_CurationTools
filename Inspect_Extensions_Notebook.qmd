...
if (nrow(inventory) > 0) {
  ...
  inventory_codebook <- tibble(
    Variable = c("FullPath", "FileName", "RelativePath", "Extension", "Size_Bytes",
                 "Size_MB", "Risk_Flag", "MIMEType", "FileType", "Author", "CreateDate",
                 "Warning", "ImageSize"),
    ...
    Description = c(
      ...
      "Pixel dimensions reported by ExifTool for image files; only present when ExifTool returns it."
    )
  )
  warn_undocumented_columns(inventory, inventory_codebook, "Full_Inventory_ExifTool")
  write_excel_csv(inventory_codebook, file.path(output_dir, "Full_Inventory_ExifTool_Codebook.csv"))

  summary_codebook <- tibble(
    Variable = c("Extension", "MIMEType", "Risk_Flag", "Count"),
    Type = c("Text", "Text", "Text", "Integer"),
    Description = c(
  ...
  write_excel_csv(summary_codebook, file.path(output_dir, "Format_Summary_Codebook.csv"))
