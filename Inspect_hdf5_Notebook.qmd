...
This notebook audits **Hierarchical Data Format version 5 (HDF5)** files (`.h5`, `.hdf5`). Unlike flat text files (CSV), [HDF5](https://support.hdfgroup.org/documentation/hdf5/latest/_h5_d_m__u_g.html) is a binary container that functions like a hard drive, capable of storing complex hierarchies of "Groups" (folders) and "Datasets" (files) within a single object.

**Preservation Risks:**

1.  **Opacity:** HDF5 is a "Black Box." Without specific software, the internal structure is invisible.
2.  **Proprietary Filters:** Data can be compressed using custom algorithms (e.g., LZF, SZIP). If the archive lacks the specific decoding plugin, the data is technically preserved but practically unreadable.
3.  **Broken Links:** HDF5 supports "External Links," where a file points to data stored in *another* file. If the linked file is missing from the transfer, the data is lost.
...
4.  **Metadata:** Extract attributes (units, descriptions) to verify the data is self-describing.
