...
This notebook inspects **Stata (`.dta`)** files. Unlike simple text formats (CSV), Stata files are "self-describing," meaning they contain both the raw data and the metadata (variable labels, value labels, and missing value codes) in a single binary container (https://www.stata-press.com/data/r18/).

**Preservation Risk:** While suitable for multiple analysis workflows, `.dta` is a proprietary format. Best practices from the **ICPSR** (Inter-university Consortium for Political and Social Research) [recommend preserving](https://www.icpsr.umich.edu/files/deposit/dataprep.pdf) these files alongside their open-source equivalents (CSV + DDI) to ensure long-term accessibility.

**Curation Objectives:**

1.  **Metadata Coverage:** Quantify the "richness" of the dataset. A file with 100 variables but 0 labels is effectively unusable for secondary analysis.
2.  **Missing Value Integrity:** Stata supports "Extended Missing Values" (e.g., `.a` for "Refused", `.b` for "Skip"). We must verify if these are present, as they will be lost if blindly converted to CSV.
