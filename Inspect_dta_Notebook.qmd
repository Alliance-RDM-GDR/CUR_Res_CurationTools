...
Quantify metadata richness and archival readiness. We want to extract embedded variable labels and "Extended Missing Values" (e.g., `.a`, `.b`) to ensure the dataset remains contextually usable.
:::
...
To facilitate curation, we extract the full schema of the dataset. This creates a codebook in CSV format, listing every variable name alongside its internal label. This allows curators to check for consistency across files (e.g., ensuring Q1 always means "Age") without opening Stata.
...
health_file <- file.path(output_dir, paste0("DTA_Health_Check_", dir_label, "_", Sys.Date(), ".csv"))
dict_file <- file.path(output_dir, paste0("DTA_Data_Dictionary_", dir_label, "_", Sys.Date(), ".csv"))
...
-   **Documentation Quality:** If Label_Coverage_Pct is low (\< 50%), the dataset may contain orphaned (undescribed and unknowable) data. Without an external codebook or data dictionary, the variables v1, v2, etc., risk losing meaningful value. Look for a codebook or data dictionary in the dataset, or request one from the depositor.

-   **Information Retention:** If the file contains Value Labels (e.g., 1=Male), converting it to a plain CSV will result in data loss (you get 1, not Male). Consider generating a setup script (R/SPSS/SAS) or using a complementary pair of formats like CSV + DDI xml to retain these mappings if you convert the stata files to csv.

-   **Privacy:** Stata files often contain string variables with open-ended survey responses. If PII_Risk is flagged, these columns should be manually reviewed for names, emails, and other identifiers.

## Additional Tools

Curators often need specialized tools to manage the transition from proprietary Stata files to open preservation formats.

-   **Stat/Transfer:** A [commercial tool](https://stattransfer.com/) for converting between statistical formats (SAS, SPSS, Stata, R). It is effective at preserving missing value definitions and variable labels during migration.

-   **Colectica / DDI:** A suite of tools for documenting data using the Data Documentation Initiative (DDI) standard. It allows you to extract Stata metadata into XML for long-term use (https://colectica.com).
