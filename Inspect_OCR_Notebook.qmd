---
title: "OCR & Document Intelligence using `daiR` and Google AI"
...
Optical Character Recognition (OCR) is the process of converting images of text (typed, handwritten, or printed) into machine-encoded text. In data curation, OCR is the bridge between "dark data" (unsearchable scanned PDFs/images) and "FAIR data" (Findable, Accessible, Interoperable, Reusable).

**Why Google Document AI?**

While standard OCR reads text line-by-line, **Document AI** uses deep learning to understand the *structure* of a document. It can distinguish between a form field (Key: "Date", Value: "2023-01-01") and a paragraph of text. The `daiR` package provides an R interface to this API [@daiR].

### Curation Goals

1.  **Digitization:** Extract text from static images/PDFs.
2.  **Quality Assessment:** Use "Confidence Scores" to flag poor scans that need manual transcription.
3.  **Privacy Screening:** Automatically scan extracted text for PII (emails, phone numbers).

## Setup

**Prerequisite:** To use `daiR`[@daiR-2], you must have a Google Cloud Project with the **Document AI API** enabled and a Service Account JSON key.
...
if (file.exists(key_path)) {
  ...
  # This usually returns a long string if successful
  tryCatch({
    token <- dai_token()
    if (!is.null(token)) {
    ...
    stop("❌ Authentication Failed: Could not get API token. Check your JSON key permissions.")
  })
  
} else {
  stop(paste("❌ Key file NOT found at:", key_path))
}
```
