---
title: "Inspection of Jupyter Notebooks (.ipynb)"
...
However, notebooks pose specific challenges for long-term preservation. Pimentel et al. (2019) analyzed over 1 million notebooks and found that **only 24%** could be re-executed without errors and only **4%** produced the same results [@pimentel2019]. Therefore, it is desirable to verify the **computational reproducibility** and **structural integrity** of jupyter notebooks.

**Key Curation Objectives:**

This notebook inspects the JSON structure of `.ipynb` files to assess three dimensions of quality:

1.  **Format Obsolescence:** Checking `nbformat` versions to ensure compatibility with modern interfaces.
2.  **Documentation Quality:** Calculating the ratio of Markdown (text) to Code cells. A low ratio indicates "spaghetti code" that will be difficult for future researchers to understand [@rule2018].
3.  **Dependency Awareness:** Scanning imports to identify the software environment (e.g., `pandas`, `tidyverse`) required for re-execution.
