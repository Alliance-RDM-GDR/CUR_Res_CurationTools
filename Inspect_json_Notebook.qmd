...
**JSON (JavaScript Object Notation)** has become the de facto standard for data interchange, replacing XML in many research workflows due to its lightweight and human-readable nature. 

::: {.callout-note title="Curation Goal"}
Validate syntactic integrity and structural complexity. Our objective is to ensure JSON files are free from trailing commas or encoding errors and to document their hierarchical depth to facilitate future data flattening and analysis.
:::

::: {.callout-warning title="Preservation Risk"}
"Structural heterogeneity"—where records in the same dataset have different fields—and extreme nesting depths without a mandatory schema often lead to data that is syntactically valid but contextually unusable [@pezoa2016].
:::

**This notebook evaluates JSON files on three levels:**

1.  **Syntactic Validity:** Can the parser read the file?
2.  **Structural Complexity:** Calculating **Nesting Depth** and **Key Count**.
3.  **Data Type Identification:** Distinguishing between Objects (single records) and Arrays.
